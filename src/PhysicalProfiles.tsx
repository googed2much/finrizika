...
function Employment({ personId }: any) {
  ...
  const handleChange = (
    c: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>,
  ) => {
  ...
  async function saveJob(c: React.SubmitEvent) {
    ...
    const response = await fetch(`/api/physical/save/employment`, {
      ...
      body: JSON.stringify({
        ...workPlace,
        startDate: workPlace
          ? workPlace.startDate.toISOString().split("T")[0]
          : null,
        endDate: workPlace.endDate
          ? workPlace.endDate.toISOString().split("T")[0]
          : null,
      }),
    });
    if (!response.ok) {
    ...
    <>
          ...
          {workList.length > 0 &&
            workList.map((job) => (
              <tr key={job.id}>
                ...
                <td>
                  {job.endDate ? job.endDate.split("T")[0] : "dabar dirba"}
                </td>
                <td>
                  {job.post === 1
                    ? "Pilnu etatu"
      ...
      <form onSubmit={saveJob}>
        ...
        <input
          ...
          onChange={handleChange}
          placeholder="darbo vietos numeris"
        />
        <label>
          Darbo pradžia
          <input
        ...
        <label>
          Darbo pabaiga - tuščia, dar dirba
          <input
            id="endDate"
            type="date"
            value={
    ...
    <>
      ...
      <form onSubmit={saveCreditForm}>
        ...
        <input
          type="number"
          id="latePaymentCount"
          placeholder="Velavimų sumokėti skaičius"
...
function EditProfile({ id }: any) {
  ...
  useEffect(() => {
    fetch(`/api/physical/get/${id}`, { method: "GET", credentials: "include" })
      .then((response) => response.json())
      .then((data) =>
        setPersonProfile1({ ...data, birthday: new Date(data.birthday) }),
      )
  ...
  async function updateProfile() {
    const response = await fetch(`/api/physical/update`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(personProfile1),
      credentials: "include",
    });
    if (response.ok) {
      alert("Pakeista");
  ...
  async function deleteProfile() {
    const response = await fetch(`/api/physical/delete/${id}`, {
      method: "DELETE",
      credentials: "include",
    });
