...
function JuridicalPortfolioAnalysis() {
  ...
  const heatmapData = evalu.map((item, index) => ({
    ...
    data: [
      {
        x: "Greitasis likvidumo koeficientas",
        y: safeNum(item.quickLiquidityPoints),
      },
      { x: "Nuosavybės koeficientas", y: safeNum(item.equityRatioPoints) },
      { x: "Palūkanų padengimas", y: safeNum(item.interestCoveragePoints) },
      { x: "Paskolų ir EBIT santykis", y: safeNum(item.netDebtRatioPoints) },
      { x: "Grynasis pelningumas", y: safeNum(item.netProfitabilityPoints) },
      { x: "Pardavimų pajamų pokytis", y: safeNum(item.salesRevenuePoints) },
    ],
  }));
    ...
    children: [
      {
        ...
        value: Math.max(
          0.1,
          avg(evalu.map((e) => safeNum(e.quickLiquidityPoints))),
        ),
      },
      {
        ...
        value: Math.max(
          0.1,
          avg(evalu.map((e) => safeNum(e.equityRatioPoints))),
        ),
      },
      {
        ...
        value: Math.max(
          0.1,
          avg(evalu.map((e) => safeNum(e.interestCoveragePoints))),
        ),
      },
      {
        ...
        value: Math.max(
          0.1,
          avg(evalu.map((e) => safeNum(e.netDebtRatioPoints))),
        ),
      },
      {
        ...
        value: Math.max(
          0.1,
          avg(evalu.map((e) => safeNum(e.netProfitabilityPoints))),
        ),
      },
      {
        ...
        value: Math.max(
          0.1,
          avg(evalu.map((e) => safeNum(e.salesRevenuePoints))),
        ),
      },
    ],
  };
  ...
  const barData = [
    {
      metric: "Greitasis likvidumo koeficientas",
      ...Object.fromEntries(
        evalu.map((e, i) => [
          `Company ${i + 1}`,
          cleanNumber(e.quickLiquidityPoints),
        ]),
      ),
    },
    {
      metric: "Nuosavybės koeficientas",
      ...Object.fromEntries(
        evalu.map((e, i) => [
          `Company ${i + 1}`,
          cleanNumber(e.equityRatioPoints),
        ]),
      ),
    },
    {
      metric: "Palūkanų padengimas",
      ...Object.fromEntries(
        evalu.map((e, i) => [
          `Company ${i + 1}`,
          cleanNumber(e.interestCoveragePoints),
        ]),
      ),
    },
    {
      metric: "Paskolų ir EBIT santykis",
      ...Object.fromEntries(
        evalu.map((e, i) => [
          `Company ${i + 1}`,
          cleanNumber(e.netDebtRatioPoints),
        ]),
      ),
    },
    {
      metric: "Grynasis pelningumas",
      ...Object.fromEntries(
        evalu.map((e, i) => [
          `Company ${i + 1}`,
          cleanNumber(e.netProfitabilityPoints),
        ]),
      ),
    },
    {
      metric: "Pardavimų pajamų pokytis",
      ...Object.fromEntries(
        evalu.map((e, i) => [
          `Company ${i + 1}`,
          cleanNumber(e.salesRevenuePoints),
        ]),
      ),
    },
  ];
  ...
  return (
    <div className={styles.page_wrapper}>
      <h2 className={styles.card_title}>JURIDINIŲ ASMENŲ PORTFELIO ANALIZĖ</h2>
      ...
      <div className={styles.dashboard}>
        <div className={`${styles.card} ${styles.heatmapCard}`}>
          ...
          <p style={{ fontSize: "12px" }}>
            Nurodomas visų įmonių duomenų rinkinys pagal kiekvieno rodiklio
            įtaką reitingui. Šviesi žemiausias reitingas, tamsi didžiausias.
          </p>
        ...
        <div className={`${styles.card} ${styles.treemapCard}`}>
          <h5>Medžio schema</h5>
          <p style={{ fontSize: "12px" }}>
            Nurodomas visų įmonių duomenų rinkinys pagal įtaka vertinimo balui
          </p>
        ...
        <div className={`${styles.card} ${styles.barCard}`}>
          ...
          <p style={{ fontSize: "12px" }}>
            Stulpelinė viso duomenų rinkinio rodklių diagrama
          </p>
          <div style={{ height: 300 }}>
            <ResponsiveBar
              data={barData}
