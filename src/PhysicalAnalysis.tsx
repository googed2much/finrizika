...
function PhysicalPortfolioAnalysis() {
    ...
    const heatmapData = evalu.map((item, index) => ({
        id: `Company ${index + 1}`,
        data: [
            { x: "Length", y: safeNum(item.lengthScore) },
            { x: "Salary", y: safeNum(item.salaryScore) },
            { x: "Lateness", y: safeNum(item.latenessScore) },
    ...
    const treeMapData = {
        ...
        children: [
            { name: "Length", value: Math.max(0.1, avg(evalu.map(e => safeNum(e.lengthScore)))) },
            { name: "Salary", value: Math.max(0.1, avg(evalu.map(e => safeNum(e.salaryScore)))) },
            { name: "Lateness", value: Math.max(0.1, avg(evalu.map(e => safeNum(e.latenessScore)))) },
            { name: "DTI", value: Math.max(0.1, avg(evalu.map(e => safeNum(e.dtiScore)))) },
        ],
    };
    ...
    const barData = [
        {
            metric: "Length",
            ...Object.fromEntries(evalu.map((e, i) => [`Company ${i + 1}`, cleanNumber(e.lengthScore)])),
        },
        {
            metric: "Salary",
            ...Object.fromEntries(evalu.map((e, i) => [`Company ${i + 1}`, cleanNumber(e.salaryScore)])),
        },
        {
            metric: "Lateness",
            ...Object.fromEntries(evalu.map((e, i) => [`Company ${i + 1}`, cleanNumber(e.latenessScore)])),
        },
        {
            metric: "DTI",
            ...Object.fromEntries(evalu.map((e, i) => [`Company ${i + 1}`, cleanNumber(e.dtiScore)])),
        },
    ];
    // visiem rodikliams
    const rawAttributes = [
    ...
    return (
        <div className={styles.page_wrapper}>
            <h2 className={styles.card_title}>
                FIZINIŲ ASMENŲ PORTFELIO ANALIZĖ
            </h2>
            ...
            <div className={styles.dashboard}>
                <div className={`${styles.card} ${styles.heatmapCard}`}>
                    ...
                    <p style={{ fontSize: "12px" }}>
                        Nurodomas visų įmonių duomenų rinkinys pagal kiekvieno rodiklio įtaką reitingui, geltona aukščiausias reitingas, tamsi žemiausias.
                    </p>
                ...
                <div className={`${styles.card} ${styles.treemapCard}`}>
                    <h5>Medžio schema</h5>
                    <p style={{ fontSize: "12px" }}>Nurodomas visų įmonių duomenų rinkinys pagal įtaką vertinimo balui</p>
                ...
                <div className={`${styles.card} ${styles.barCard}`}>
                    ...
                    <p style={{ fontSize: "12px" }}>Stulpelinė viso duomenų rinkinio rodiklių diagrama</p>
                    <div style={{ height: 300 }}>
                        <ResponsiveBar
                            data={barData}
                ...
                <div className={`${styles.card} ${styles.sidePanelCard}`}>
                    ...
                    <p style={{ fontSize: "12px", marginBottom: 10, marginTop: 4 }}>
                        Nurodomi visų vertinimui naudojamų rodiklių histogramos, dėžutės diagramos ir skirstinio funkcijos grafikai
                    </p>
